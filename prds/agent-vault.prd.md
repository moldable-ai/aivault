# Agent Vault — Zero-Trust Credentials For Any Code
...
```bash
agent-vault credential create openai --secret sk-abc123
```
...
```bash
curl -sS "$AGENT_VAULT_BASE_URL/agent-vault/proxy" \
...
```bash
agent-vault credential create google --secret <oauth-json>
```
...
`agent-vault` fixes this by putting a broker between any code and the APIs it needs. The code never sees the real key — it gets a scoped token that can only talk to the local broker. If it exfiltrates that token, it doesn't matter, because the token is worthless outside the local broker. The broker holds the real keys, enforces what each capability is allowed to do, and only proxies requests to an explicit allow-list of hosts — so none of the above attacks work even if the code is fully compromised.
...
```bash
agent-vault credential create openai-work     --provider openai --secret sk-work-key
agent-vault credential create openai-personal  --provider openai --secret sk-personal-key
```
...
```bash
agent-vault credential create my-api \
  --provider my-api \
  --auth-type header \
...
agent-vault capability create my-api/users \
  --provider my-api \
  --methods GET POST \
  --paths /v2/users
```
...
- `AGENT_VAULT_BASE_URL` — the broker base URL
- `AGENT_VAULT_TOKEN` — scoped capability token for this execution
...
### Proxy request (`POST /agent-vault/proxy`)
...
- `POST/GET /agent-vault/credentials`
- `GET/PATCH/DELETE /agent-vault/credentials/:id`
- `POST/GET /agent-vault/capabilities`
- `GET/PATCH/DELETE /agent-vault/capabilities/:id`
- `POST /agent-vault/tokens/proxy`
...
`POST /agent-vault/tokens/proxy` request:
...
- `POST/GET /agent-vault/secrets`
- `GET/PATCH/DELETE /agent-vault/secrets/:id`
- `POST /agent-vault/secrets/:id/rotate`
...
### Envelope mode (`POST /agent-vault/proxy`)
...
### WebSocket proxy (`GET /agent-vault/ws`)
...
Defense: Provider secrets are never in the caller's environment. The caller only receives `AGENT_VAULT_BASE_URL` and `AGENT_VAULT_TOKEN`. The proxy token is worthless outside the local broker.
...
Attack: Code saves its `AGENT_VAULT_TOKEN` to disk for use in a later execution, or passes it to another process.
...
- Envelope mode: `POST /agent-vault/proxy` with `capability` + `request.path` (no caller-supplied upstream URL).
...
- `GET /agent-vault/ws` WebSocket proxy.
...
To keep adoption simple, `agent-vault` defines additive conformance levels. Implementers can ship a secure core quickly and add convenience features later.

- `Core`: credentials + capabilities, envelope mode (`POST /agent-vault/proxy` with `request.path`), passthrough mode (`/v/{credential}/...`), `allow.hosts` required (exactly one host), allow-list enforcement, reserved header stripping, SSRF protections, localhost-only binding by default, structured broker errors.
...
- `WebSocket`: `GET /agent-vault/ws` with connect frame (`capability` + `path`) + frame relay.
...
- [x] Caller contract via `AGENT_VAULT_BASE_URL` and `AGENT_VAULT_TOKEN` is covered by [`env-caller-env-vars-contract`](./user-stories/proxy-envelope.json).
